...
function UpdateStoreDialog({ open, onClose, store, ...rest }) {
  const classes = useStyles();
  const [storeImg, setStoreImg] = useState();
  const { updateStore } = useOffer();
    ...
    if (storeImg) {
      handleDelete(values.imgRef).then(() => {
        handleUpload(storeImg).then((imgInfo) => {
          updateStore(store.id, values, imgInfo).then((id) => {
            console.log(id);
            onClose();
          });
        });
      ...
      updateStore(store.id, values).then((id) => {
        console.log(id);
        onClose();
      });
    }
  ...
  return (
    <>
      <Dialog open={open} onClose={onClose} aria-labelledby="form-dialog-title">
        <DialogTitle id="form-dialog-title">Edit store details</DialogTitle>
        <DialogContent>
          <DialogContentText>
